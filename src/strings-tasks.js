...
function isString(value) {
  // throw new Error('Not implemented');
  // if (typeof valueOf(value) !== 'string') {
  //   return false;
  // }
  // return true;
...
function formatTime(minutes, seconds) {
  // throw new Error('Not implemented');
  // let nik = minutes.padStart(2, '0')
  // let steave = seconds.padEnd(2, '0')
...
function countVowels(str) {
  ...
  return str
    .split('')
    .filter((x) => (x.match('[aeiuoyAEIUOY]') ? sum + 1 : sum))
    .join('').length;
}
...
function isPalindrome(str) {
  // throw new Error('Not implemented');
  const nik = str.replaceAll(/[\W_]/g, '').toLowerCase();
  const nik2 = str
    .replaceAll(/[\W_]/g, '')
    .toLowerCase()
    .split('')
...
function invertCase(str) {
  // throw new Error('Not implemented');
  const newStr = str.replace(/[A-Za-z]/g, function tom(char) {
